...
namespace Nakama
{
    public class NakamaClient : MonoBehaviour
    {
        ...
        public async Task<ISocket> GetSocket()
        {
        ...
        public async Task<string> EnterGame(string myIp)
        {
            try 
            {
                var rpcResponse = await _client.RpcAsync(_session, "get_or_create_match", myIp);
                var result = JsonUtility.FromJson<RpcMatchResponse>(rpcResponse.Payload);
